	...
	void Update () {
		if (GameManager.prevState.IsConnected) {
			if (GameManager.prevState.Buttons.RightShoulder == ButtonState.Pressed && GameManager.state.Buttons.RightShoulder == ButtonState.Released) {
				...
				if (ControllerSelection == 0 && Frame_Number < 2) {
					NextFrame ();
				} else if (ControllerSelection == 1 && RWeapon_Number < 2) {
					RNextWeapon (); 				
				} else if (ControllerSelection == 2 && LWeapon_Number < 2) {
					LNextWeapon ();
				}
			...
			if (GameManager.prevState.Buttons.LeftShoulder == ButtonState.Pressed && GameManager.state.Buttons.LeftShoulder == ButtonState.Released ) {
				...
				if (ControllerSelection == 0 && Frame_Number > 0 ) {
					PreviousFrame ();
				} else if (ControllerSelection == 1 && RWeapon_Number > 0) {
					RPreviousWeapon ();
				} else if (ControllerSelection == 2 && LWeapon_Number > 0 ){
					LPreviousWeapon (); 				
				}
			}
